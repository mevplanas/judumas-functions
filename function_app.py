...
#         chunks = utils.create_chunks(n=500, data=entities_converted)
#         i = 0
    ...
    for input_file in req.files.values():
        ...
        if filename.endswith(".zip"):
            ...
            if filename.endswith("_ride.zip"):
                blob_handler.upload_blob_zip(
                    content, container="strava/bikes", blob_name=filename
                )
            if filename.endswith("_ped.zip"):
                blob_handler.upload_blob_zip(
                    content, container="strava/pedestrians", blob_name=filename
                )
...
def strava_mapper(myblob: func.InputStream):
    ...
    if myblob.name.endswith(".zip"):
        ...
        chunks = utils.create_chunks(4000, data_dict)
        blob_handler = az.OpBlobHandler()
        i = 0
        for chunk in chunks:
...
@app.blob_trigger(arg_name="myblob", path="strava-csv", connection="judumas_STORAGE")
def strava_csv_trigger(myblob: func.InputStream):
    #
    queue_client = az.OpQueuesHandler("strava-insert-procedure")
    # Create blob name
    blob_name = myblob.name.split("/")[1]
    queue_client.insert_queque_msg(f"{blob_name}")
